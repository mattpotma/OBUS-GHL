...
from ghlobus.utilities.sample_utils import subsample_frames


class VideoDatasetTraining(VideoDatasetBase):
    """
    A flexible training dataset that can be configured to handle different
    ...
    Parameters:
        ...
        subsample (Callable, optional):    Function for frame subsampling.
                                           Defaults to None.
        frames (int):                      Number of frames to use for training (random sampling).
    """
    ...
    def __init__(self,
        ...
        self.frames = frames
        # We are always subsampling frames in training, for memory reasons
        if self.subsample is None or not callable(self.subsample):
            self.subsample = lambda x: subsample_frames(x, k=self.frames)

    def _process_frames(self, frames: Tensor) -> Tensor:
        """
        Applies training-specific processing to frames.
        """
        return frames
